...
Уточнение после review реализации от 06.09.2026: исходные Context/Existing code ниже относятся к подготовке задачи. Major-замечания к коммиту `18969eb` исправляются отдельным изменением: сохраняется подписка session cache, подключаются OpenTelemetry SDK/private Prometheus export и исполняемые dashboard/alert configurations. Формулировка прежнего observability-документа об отложенном exporter заменена реализацией; production tenant и адресаты настраиваются оператором по ADR-014. Это не объявляет остальные замечания review и все acceptance gates завершёнными.
