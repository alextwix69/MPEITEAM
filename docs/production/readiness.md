...
- [ ] Опубликованы утверждённые тексты и версии четырёх документов регистрации; значения `local-v1` удалены из production configuration.
- [ ] Выбран production email provider и подтверждена его дедупликация по стабильному `Message-ID`.
